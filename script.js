...
function renderProducts(){const grid=document.getElementById("productGrid");const list=products.filter(p=>p.active!==false&&(currentFilter==="all"||p.category===currentFilter));grid.innerHTML=list.map(p=>`<article class="product"><div class="product-img">${p.image?`<img src="${p.image}" alt="${p.name}">`:`<div class="placeholder">YOUR<br>BRAND</div>`}</div><div class="product-info"><h3>${p.name}</h3><div class="muted">${p.category==="oversize"?"Oversize":"Basic"} · ${p.stock>0?"متوفر":"غير متوفر"}</div><div class="price">${p.price} ج.م</div><button class="add" ${p.stock<=0?"disabled":""} onclick="addToCart(${p.id})">أضف للسلة</button></div></article>`).join("")}
...
function checkout(){if(!cart.length)return alert("أضف منتجًا للسلة أولًا.");const phone="201001648519";const lines=cart.map(i=>{const p=products.find(x=>x.id===i.id);return `- ${p.name} × ${i.qty} = ${p.price*i.qty} ج.م`});const total=cart.reduce((s,i)=>s+products.find(p=>p.id===i.id).price*i.qty,0);const msg=`مرحبًا، أريد طلب:\n${lines.join("\n")}\nالإجمالي: ${total} ج.م`;window.open(`https://wa.me/${phone}?text=${encodeURIComponent(msg)}`,"_blank")}
