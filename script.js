...
function renderCart(){
  ...
  let total=0,count=0;
  if(!cart.length) {
    box.innerHTML="<p class='muted'>السلة فاضية حاليًا.</p>";
  } else {
    box.innerHTML=cart.map(item=>{
      ...
      count+=item.qty;
      return `<div class="cart-row"><div class="mini">VYRO</div><div><b>${p.name}</b><div class="muted">${p.price} ج.م</div></div><div class="qty"><button onclick="changeQty(${p.id},-1)">−</button>${item.qty}<button onclick="changeQty(${p.id},1)">+</button></div></div>`;
    }).join("");
  }
...
function submitOrder(event){
  ...
  const text=`طلب جديد%0Aالاسم: ${name}%0Aالهاتف: ${phone}%0Aالعنوان: ${address}%0Aالمنتجات: ${items}%0Aالإجمالي: ${total} ج.م`;
  window.open(`https://wa.me/201001648519?text=${text}`,"_blank");
